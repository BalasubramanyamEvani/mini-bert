...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        # query, key, value dimension: [bs, num_attention_heads, seq_len, seq_len]
        ...
        V = V.transpose(1, 2)
        V = V.reshape((bs, seq_len, self.all_head_size))
...
class BertLayer(nn.Module):
    ...
    def add_norm(self, input, output, dense_layer, dropout, ln_layer):
        ...
        """
        # todo
        ff_output = dropout(dense_layer(output))
        add_norm_output = ln_layer(input + ff_output)
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # feed forward
        output = self.interm_dense(output_add_norm)
